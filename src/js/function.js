...
function TabDisplay(tab, tabContent, options) {
    this.tab = null;
    ...
    this.mount = () => {
        ...
        if (isDOM(tabContent)) {
            this.tabContent = tabContent;s
        }
        else {
            this.tabContent = document.querySelector(tabContent);
        }
        if (isLiteralObject(options) && !isEmpty(options)) {
            this.setOptions(options);
        }
        if (this.tab !== null && this.tabContent !== null) {
            registerTabButton(this.tab, this.tabContent);
            if (this.options.autoSize) {
                tabContentListFixedSize(this.tabContent);
            }
            // resize on screen resize
            window.addEventListener('resize', (event) => {
                setTimeout(() => {
                    // set dimension to auto for reevaluation
                    if (this.options.autoSize) {
                        css(this.tabContent, {
                            'height': `auto`,
                            'width': `auto`,
    ...
    this.setOptions = (options) => {
        this.options.autoSize = options.autoSize !== undefined ? options.autoSize : this.options.autoSize;
    }
    function registerTabButton(tab, tabContent) {
        let tabChildren = tab.children;
        let contentChildren = tabContent.children;
        for (let i = 0; i < tabChildren.length; i++) {
            // check if the li > button exist
            if (tabChildren[i].children[0].nodeName === 'BUTTON') {
                // bind action to button
                tabChildren[i].children[0].addEventListener('click', (event) => {
                    // jQuery
                    $(event.target).addClass('active'); // target : button
                    $(tabChildren[i]).siblings().children().removeClass('active');
                    $(contentChildren[i]).addClass('active').siblings().removeClass('active');
                });
            }
        }
    }
    function tabContentListFixedSize(tabContent) {
        let contentChildren = tabContent.children;
        let maxHeight = 0;
        let maxWidth = 0;
        // assume the first child is the highest dimension
        let rect = contentChildren[0].getBoundingClientRect();
        maxHeight = rect['height'];
        maxWidth = rect['width'];
        if (contentChildren.length > 1) {
            for (let i = 1; i < contentChildren.length; i++) {
                let rect = contentChildren[i].getBoundingClientRect();
